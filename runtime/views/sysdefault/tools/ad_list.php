...
<div class="content">
	<form name='ad' method='post' action='<?php echo IUrl::creatUrl("/tools/ad_del");?>'>
		<table id="list_table" class="list_table">
			...
			<tbody>
				<?php $query = new IQuery("ad_manage as ad");$query->join = "left join ad_position as adp on ad.position_id = adp.id";$query->fields = "adp.name as adp_name,ad.*";$query->order = "ad.order ASC";$items = $query->find(); foreach($items as $key => $item){?>
				<tr>
					...
					<td>
						<a href='<?php echo IUrl::creatUrl("/tools/ad_edit/id/".$item['id']."");?>'><img class="operator" src="<?php echo $this->getWebSkinPath()."images/admin/icon_edit.gif";?>" alt="编辑" title="编辑" /></a>
						<a href='javascript:void(0)' onclick="delModel({link:'<?php echo IUrl::creatUrl("/tools/ad_del/id/".$item['id']."");?>'});"><img class="operator" src="<?php echo $this->getWebSkinPath()."images/admin/icon_del.gif";?>" alt="删除" title="删除" /></a>
					</td>
				</tr>
				<?php }?>
			</tbody>
